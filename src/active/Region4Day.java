...
public class Region4Day {
	...
	public static void main(String[] args) throws Exception {
		...
		query = "SELECT '"+dateValue+"' as datetime, region, city, count(distinct(deviceidentifier)) as num "
		...
		ResultSetHandler<List<SumRegionByDay>> regionHandler = new BeanListHandler<SumRegionByDay>(SumRegionByDay.class);
		List<SumRegionByDay> regions = run.query(conn, query, regionHandler, datesk);
...
//			System.out.println(region.getDatetime()+" "+region.getRegion()+" " + region.getCity()+ " " + region.getNum());
			run.update(conn, query, region.getDatetime(),region.getRegion(),region.getCity(),region.getNum());
		}
